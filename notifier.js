(function(){
  ...
  function createElement(type, styles) {
    ...
    if (styles) setStyle(elm, styles)
    return elm
  }
  function fadeOut(element, duration, cb) {
    var interval, start = Date.now()

    interval = setInterval(function() {
      var pos = (Date.now() - start) / duration
      if (pos > 1) pos = 1

      element.style.opacity = (1 - pos)
  ...
  function init() {
    ...
    style[ config.position[0] ] = '12px'
    style[ config.position[1] ] = '12px'

    setStyle(config.container, style)
  ...
  Notifier.notify = function(message, title, iconUrl, timeOut) {
    ...
    if (message) {
      var messageElement = createElement('div')
